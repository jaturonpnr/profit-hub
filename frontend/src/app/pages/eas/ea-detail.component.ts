...
interface EaDetail {
  ...
  avgExecutionMs: number | null; maxExecutionMs: number | null;
    ...
    <div class="animate-fade-in flex flex-col gap-6">
      ...
      @else if (d() !== null) { @if (d(); as ea) {
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
          ...
          <div class="rounded-lg border border-border bg-surface p-4"><div class="text-xs text-text-faint">Avg exec</div><div class="mt-1 text-lg font-semibold tabular-nums">{{ ea.avgExecutionMs != null ? (ea.avgExecutionMs | number:'1.0-0') + ' ms' : '—' }}</div></div>
          <div class="rounded-lg border border-border bg-surface p-4"><div class="text-xs text-text-faint">Max exec</div><div class="mt-1 text-lg font-semibold tabular-nums">{{ ea.maxExecutionMs != null ? (ea.maxExecutionMs | number:'1.0-0') + ' ms' : '—' }}</div></div>
        </div>
